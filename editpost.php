...
if($_GET['action'] == "err" && !empty($_GET['id'])){
            ...
            $query = mysqli_query($db,"DELETE FROM forum_post WHERE id = '{$_GET['id']}'");
            $query2 = mysqli_query($db,"DELETE FROM forum_replies WHERE post = '{$_GET['id']}'");

            if($query){

            $good2 = "Post borrada correctamente";
            header("LOCATION: ".$rise_settings['weblink']."/forum");   
            }                  
    }
 ...
 if(isset($_GET['id'])){
    if($_GET['id'] && !empty($_GET['id'])){
    $hj = mysqli_query($db,"SELECT * FROM forum_post WHERE id = '{$_GET['id']}'");
...
<head>
    ...
    <title><?php echo $rise_settings['name'];?> - Nuevo Tema</title>
...
if ((isset($_POST['update']))) {
    ...
    $date_full = time();
     ...
     $result = mysqli_query($db,"UPDATE forum_post SET title = '".$title."', content = '".$content."' WHERE id = '".$h_edit['id']."' LIMIT 1"); 
                                        if ($result) {
            ...
            <div class="notification-content">
                <font style="vertical-align: inherit;"><font style="vertical-align: inherit;">Actualizaste este post.</font></font>
            </div>
        </div>
            </div>';
                                        } 
...
<section class="page-container" data-page="foro">
	...
	<div class="page-content">
        <?php require_once "templates/foro/edit_post.php";?>
	</div>
</section>
