...
if($_GET['action'] == "err" && !empty($_GET['id'])){
            ...
            $query = mysqli_query($db,"DELETE FROM badges WHERE id = '{$_GET['id']}'");

            if($query){

            $good2 = "Post borrada correctamente";
            header("LOCATION: ".$rise_settings['weblink']."/admin/badges.php");   
            }                  
    }
...
<head>
    ...
    <title><?php echo $rise_settings['name'];?> - Crear / Borrar Placas</title>
...
<body>
	...
	<div class="container">
        ...
        <div class="col-8">
            <div class="alert success">Placas de <?php echo $rise_settings['name'];?></div>

            <div id="content-box" style="height:380px">
                <div class="title-box png20">
                    <div class="title">Publicar una Placa</div>
                </div>
...
if ((isset($_POST['create']))) {
    ...
    $code = $_POST['code'];
     ...
     $result = mysqli_query($db,"INSERT INTO `badges` (`code`) VALUES ('".$code."');");
                                        if ($result) {
                                        echo '<div class="botonsce">Placa publicada con exito</div>';
                                        } 
                    }
?> 
<form action="" method="POST">
  <input type="text" name="code" placeholder="Codigo de la Placa">
  <input type="submit" name="create" class="botonsc" value="Publicar Placa">
</form>
             <div class="title-box png20">
                    <div class="title">Lista de Placas</div>
                </div>
          <table style="width: 100%;">
              <tbody>
                  ...
                  <?php
 $news = mysqli_query($db, "SELECT * FROM badges ORDER BY id DESC");
    while ($newsc = mysqli_fetch_assoc($news)) {
                  ?>
                  <tr>
                      <td><?php echo $newsc['code'];?></td>
                      <td><a href="<?php echo $rise_settings['weblink'];?>/admin/badges.php?action=err&id=<?php echo $newsc['id'];?>" style="color:#FA5858;text-decoration: none;">Borrar</a></td>
                  </tr>
<?php } ?>
              </tbody>
          </table>
