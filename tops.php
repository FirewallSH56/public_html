...
<head>
    ...
    <title><?php echo $rise_settings['name'];?> - Top Usuarios</title>
...
<section class="page-container" data-page="games_ranking">
	<header class="page-header flex-container flex-vertical-center">
		<div class="page-header-content"><h1>Top Usuarios</h1> Aquí encontraras a los usuarios con mas juegos ganados y con mas flamas.</div>
	</header>
				...
				<table class="default-table ranking-content">
					<thead>
						<tr>
							...
							<th>Games Win <i class="fas fa-gamepad"></i></th>
						</tr>
					</thead>
				<tbody>
				<?php 
    $events = mysqli_query($db, "SELECT * FROM users ORDER BY points ASC LIMIT 5");
    while ($eventc = mysqli_fetch_assoc($events)) {
        ?>
								<tr>
					<td>
					<div class="account-avatar"><img src="https://habbo.it/habbo-imaging/avatarimage?figure=<?php echo $eventc['look'];?>&action=std&gesture=std&direction=2&head_direction=2&size=n&img_format=png" alt="" class="pixelated"></div>
					</td>

					<td><a href="<?php echo $rise_settings['weblink'];?>/profile/<?php echo $eventc['username'];?>"><?php echo $eventc['username'];?></a></td>
					<td><span><?php echo $eventc['points'];?></span></td>
				</tr>
			<?php } ?>
			</tbody>
		</table>
			...
			<article class="default-section ranking-container">
				<table class="default-table ranking-content">
					<thead>
						<tr>
							<th colspan="2">Usuario</th>
							<th>Flamas <i class="fas fa-fire"></i></th>
						</tr>
					</thead>
				<tbody>
				
									<?php 
    $events = mysqli_query($db, "SELECT * FROM users ORDER BY pixels ASC LIMIT 5");
    while ($eventc = mysqli_fetch_assoc($events)) {
        ?>
								<tr>
					<td>
					...
					<td><a href="<?php echo $rise_settings['weblink'];?>/profile/<?php echo $eventc['username'];?>"><?php echo $eventc['username'];?></a></td>
					<td><span><?php echo $eventc['pixels'];?></span></td>
				</tr>
			<?php } ?>
				</tbody>
			</table>
